...
**UI:** shadcn/ui (Radix, "default" style, lucide icons) in `src/components/ui` — add components via the shadcn CLI per `components.json`. Theme tokens use a three-layer system in `src/app/globals.css` (primitives like `--coral-600` → shadcn semantic names like `--primary` → component tokens like `--nav-height`); read `docs/design-system.md` before touching colors. Components use only semantic Tailwind classes (never raw hex or primitives), and hover on text-bearing colors uses the solid `*-hover` tokens, not `/90` opacity. Dark mode follows the OS via a `<head>` script in `layout.tsx` that toggles `.dark`. Charts use Recharts via `components/ui/chart.tsx`. Remote images are only allowed from `placehold.co` (used as the fallback `imagen_url`); local clothing photos are served from `public/ropa/` (`public/ropa/antes/` holds the un-optimized originals).
